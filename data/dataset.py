...
def get_datasets(data_name, dataroot, normalize=True, val_size=10000, one_hot=True):
    ...
    if data_name == 'cifar10':
        normalization = transforms.Normalize((0.5, 0.5, 0.4), (0.2, 0.2, 0.2))
        data_obj = CIFAR10
        num_classes = 10
    elif data_name == 'cifar100':
        normalization = transforms.Normalize((0.5, 0.5, 0.4), (0.3, 0.3, 0.3))
        data_obj = CIFAR100
        num_classes = 100
    else:
...
def gen_classes_per_node(dataset, num_users, classes_per_user=2, high_prob=0.5, low_prob=0.5):
    ...
    #count_per_class = max(1, classes_per_user * num_users // num_classes)
    count_per_class = math.ceil(classes_per_user * num_users / num_classes)
    class_dict = {}
    ...
    for i in range(num_users):
        ...
        for _ in range(classes_per_user):
            ...
            while True:
                class_ = np.random.choice(max_class_counts)
                if class_ not in set(c):
                    c.append(class_)
                    break
...
def gen_random_loaders(data_name, data_path, num_users, num_train_users, bz,
                       ...
                       embedding_dir_path=None, seed=42, do_mapping = False):
    """
    ...
    one_hot = False if do_mapping else True
    ...
    if data_name == 'femnist':
        ...
        dataloaders = []
        datasets = get_datasets(data_name, data_path, normalize=True, one_hot=one_hot)
        label_mappings = None
        class_partitions = None
        for i, d in enumerate(datasets):
            ...
            class_partitions = cls_partitions['class']
            #if train with output dim num_classes:
            if do_mapping:
                label_mappings = [get_label_mapping(subset) for subset in subsets] if label_mappings is None else label_mappings
                subsets = [remap_labels(subset, label_mappings[i]) for i,subset in enumerate(subsets)]
            
            # create dataloaders from subsets
            dataloaders.append(list(map(lambda x: torch.utils.data.DataLoader(x, **loader_params), subsets)))
        
        train_loaders, test_loaders = dataloaders
        train_loader = [{'train':train_loaders[i] ,'test':test_loaders[i], 'partitions': list(map(int,class_partitions[i]))} for i in range(num_train_users)]
        test_loader = [{'train': train_loaders[num_train_users+i],'test': test_loaders[num_train_users+i], 'partitions': list(map(int, class_partitions[num_train_users+i]))} for i in range(num_users - num_train_users)]
...
def remap_labels(dataset, label_map):
    ...
    remapped_targets = [label_map[label] for label in targets]
    if isinstance(dataset, torch.utils.data.Subset):
        dataset.dataset.targets = list(np.array(dataset.dataset.targets))
        for idx, mapped_label in zip(dataset.indices, remapped_targets):
            dataset.dataset.targets[idx] = mapped_label
    else:
