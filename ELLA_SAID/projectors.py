...
class SAIDModule(nn.Module):
    ...
    def __init__(self, net, projector, basis_num, dimension=1000, global_param = None, global_lambdas=None, task_lambdas=None, global_P = None, is_lambda_param = True, is_last_layer_only=False, is_lambda_global=False):
        super().__init__()
        ...
        self.D = sum([param.numel() for param in [p for n, p in initnet.named_parameters() if p.requires_grad]])
        ...
        self.global_lambdas = global_lambdas if is_lambda_global else (nn.Parameter(torch.ones(self.said_size)/math.sqrt(self.said_size),requires_grad = True) if is_lambda_param else (last_layer_lambda if is_last_layer_only else torch.ones(self.said_size)))
        ...
        self.alphas = nn.Parameter(torch.ones(self.basis_num)/self.basis_num)
        ...
        self.lambdas = task_lambdas if is_lambda_global else (nn.Parameter(torch.ones(self.said_size)/math.sqrt(self.said_size)) if is_lambda_param else (last_layer_lambda if is_last_layer_only else torch.ones(self.said_size)))
    ...
    def forward(self, *args, **kwargs):
        ...
        hyper_dim = int(len(self.global_param) / self.basis_num)
        coefficients = torch.repeat_interleave(self.alphas, hyper_dim).to(self.subspace_params.device)
        weights = coefficients * self.global_param

        flat_projected_global_params = self.global_P @ weights
        ...
        unflattened_global = unflatten_like(flat_projected_global_params, self.trainable_initparams)
        ...
        for i,p_name, init, proj_param, proj_global in iterables:
            p = init + self.global_lambdas[i]*(proj_global.view(*init.shape)) + (self.lambdas[i] * proj_param.view(*init.shape))
...
def create_intrinsic_model(
    ...
    if intrinsic_mode == "filmrdkron":
        class FiLMRDKronIDNet(module):
            def __init__(self, net, basis_num = basis_num, dimension=1000, seed=None,global_param = global_param, global_lambdas=global_lambdas, task_lambdas=task_lambdas, global_P = global_P, is_lambda_param=is_lambda_param, is_last_layer_only=is_last_layer_only, is_lambda_global=is_lambda_global, **_):
                super().__init__(
                    net, projector = partial(CombinedRDKronFiLM, seed=seed,is_layered = True), dimension=dimension , basis_num = basis_num,global_param = global_param, global_P = global_P, is_lambda_param=is_lambda_param, is_last_layer_only=is_last_layer_only, global_lambdas=global_lambdas, task_lambdas=task_lambdas, is_lambda_global=is_lambda_global
                )
        net = FiLMRDKronIDNet(base_net, basis_num = basis_num, dimension=intrinsic_dim, seed=seed,global_param = global_param, global_P = global_P, is_lambda_param=is_lambda_param, is_last_layer_only=is_last_layer_only, global_lambdas=global_lambdas, task_lambdas=task_lambdas, is_lambda_global=is_lambda_global
                             )
