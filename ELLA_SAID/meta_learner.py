...
from pactl.nn import create_model, FcNet3, ConvNet3,OmConvNet_NoBN
from ELLA_SAID.projectors import create_intrinsic_model,CombinedRDKronFiLM
...
from ELLA_SAID.quantize_lambdas import quantize_lambdas
import copy
from copy import deepcopy
from functools import partial


class ELLA_SAID(nn.Module):
  def __init__(self, train_loader,test_loader,intrinsic_dim, basis_num, model_name=None, base_width=None,
                   seed=None, intrinsic_mode=None, all_train_task_levels=20, is_lambda_global=False, lambdas_levels=3,
                   ...
                   quantize_type = 'default', hyper_dim = None, global_levels=20, is_lambda_param = True, is_last_layer_only = False):
        super().__init__()
        ...
        self.compression_args = {'levels': levels, 'global_levels': global_levels, 'use_kmeans': use_kmeans, 'misc_extra_bits': misc_extra_bits, 'epochs': quant_epochs, 'lr': quant_lr,
                   'global_quantize_type': quant_types[0], 'quantize_type': quant_types[1], 'all_train_task_levels':all_train_task_levels, 'lambdas_levels': lambdas_levels}
        

        self.intrinsic_net_args = {'ckpt_path':None, 'intrinsic_mode':intrinsic_mode, 'intrinsic_dim':intrinsic_dim, 'seed':seed,
          'device':device_id, 'basis_num' : basis_num, 'is_lambda_param':is_lambda_param, 'is_last_layer_only':is_last_layer_only, 'is_lambda_global': is_lambda_global}
        ...
        self.said_size = len([p for n,p in self.base_net.named_parameters() if p.requires_grad]) if is_lambda_global or is_lambda_param else 0

        self.basis_num = basis_num
        hyper_dimension = intrinsic_dim if hyper_dim is None else hyper_dim
        self.d = hyper_dimension 
        self.global_param = nn.Parameter(torch.ones(self.d * self.basis_num)/self.d)

        ####global lambdas
        self.global_lambdas = nn.Parameter(torch.ones(self.said_size)/math.sqrt(self.said_size)) if is_lambda_global else None
        self.task_lambdas = nn.Parameter(torch.ones(self.said_size)/math.sqrt(self.said_size)) if is_lambda_global else None
        ...
        self.nets = [create_intrinsic_model(copy.deepcopy(self.base_net), **self.intrinsic_net_args, global_param = self.global_param, global_lambdas=self.global_lambdas, task_lambdas=self.task_lambdas, global_P = self.global_P).to(device_id) for i in range(self.num_train_tasks)]
        self.test_nets = None
  ...
  def meta_train(self, criterion, optimizer, lr, device_id, epochs, log_dir, weight_decay=5e-4):
    ...
    params = [self.global_param, self.global_lambdas, self.task_lambdas] if self.intrinsic_net_args['is_lambda_global'] else [self.global_param]

    for i in range(self.num_train_tasks):
      params_list= [self.nets[i].alphas, self.nets[i].subspace_params] if self.intrinsic_net_args['is_lambda_global'] or (not self.intrinsic_net_args['is_lambda_param']) else [self.nets[i].alphas, self.nets[i].subspace_params, self.nets[i].lambdas, self.nets[i].global_lambdas]
      params+= params_list
  ...
  def train_eval_test_tasks(self,lr,device_id,criterion,epochs, weight_decay=5e-4):
    ...
    if self.intrinsic_net_args['is_lambda_global']:  
      #quantize global and task lambas
      quantized_lambdas, quantized_lambdas_message_len = quantize_lambdas(quant_type = self.compression_args['global_quantize_type'], meta_learner = self, levels = self.compression_args['lambdas_levels'], device = self.net_args['device_id'], epochs = self.compression_args['epochs'], lr = self.compression_args['lr'], use_kmeans = self.compression_args['use_kmeans']) 
      quantized_global_message_len += quantized_lambdas_message_len

      self.global_lambdas = deepcopy(torch.tensor(quantized_lambdas[0],dtype=torch.float16).float().to(device_id))
      self.task_lambdas = deepcopy(torch.tensor(quantized_lambdas[1],dtype=torch.float16).float().to(device_id))
      self.global_lambdas.requires_grad = False
      self.task_lambdas.requires_grad = False

      for j in range(self.num_train_tasks):
        self.nets[j].global_lambdas.data = self.global_lambdas
        self.nets[j].global_lambdas.requires_grad = False

        self.nets[j].lambdas.data = self.task_lambdas
        self.nets[j].lambdas.requires_grad = False
    ...
    self.test_nets = [create_intrinsic_model(copy.deepcopy(self.base_net), **self.intrinsic_net_args, global_param =  self.global_param, global_lambdas=self.global_lambdas, task_lambdas=self.task_lambdas, global_P = self.global_P).to(device_id) for i in range(self.num_test_tasks)]
    test_task_bounds = []
  ...
  def compress(self, quant_type, net, levels, device, train_loader, epochs, lr, use_kmeans):
    if quant_type =='default_all_tasks':
      quant_type = 'default_float8' if self.intrinsic_net_args['is_lambda_global'] or (not self.intrinsic_net_args['is_lambda_param']) else 'default_float8_float8_float8'

    # subspace-params_alphas
    ...
    ###quantizing alphas
    quantized_vecs, len = quantize(types[1], net, ['alphas'], levels, device, train_loader, epochs, lr, use_kmeans)
    net.alphas.data = torch.tensor(quantized_vecs[0],dtype=torch.float16).float().to(device)  
    message_len += len  
    
    
    if self.intrinsic_net_args['is_lambda_param'] and (not self.intrinsic_net_args['is_lambda_global']):
      if types[2]=='together':
        quantized_vecs, len = quantize(types[3], net, ['lambdas', 'global_lambdas'], levels, device, train_loader, epochs, lr, use_kmeans)
        ...
        quantized_vecs, len = quantize(types[2], net, ['lambdas'], levels, device, train_loader, epochs, lr, use_kmeans)
        net.lambdas.data = torch.tensor(quantized_vecs[0],dtype=torch.float16).float().to(device) 
        message_len += len
        quantized_vecs, len = quantize(types[3], net, ['global_lambdas'], levels, device, train_loader, epochs, lr, use_kmeans)
  ...
  def compress_all_tasks(self):
    #lambda_global,lambda_local,alphas,subspace_params
    ...
    #alphas
    quantized_vecs, message_len = compute_quantization_all_tasks(self, 'alphas', self.compression_args['all_train_task_levels'], self.net_args['device_id'], self.compression_args['epochs'], self.compression_args['lr'], self.compression_args['use_kmeans'])
    sum_task_message_len+= message_len

    for i in range(self.num_train_tasks):
      if quantized_vecs[i] is not None:
        self.nets[i].alphas.data = torch.tensor(quantized_vecs[i],dtype=torch.float16).float().to(self.net_args['device_id'])
