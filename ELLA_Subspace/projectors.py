...
class IDModule(nn.Module):
    ...
    def __init__(self, net, basis_num, dimension=1000, global_param = None, global_P = None):
        super().__init__()
        self.d = dimension
...
class AIDModule(nn.Module):

    def __init__(self, net, basis_num, dimension=1000, global_param = None, global_P = None):
        super().__init__()
        self.d = dimension
        self.D = sum([param.numel() for param in [p for n, p in net.named_parameters() if p.requires_grad]])

        self.basis_num = basis_num
        self.global_param = global_param
        self.global_P = global_P
        
        self.w = nn.Parameter(torch.ones(self.d)/self.d, requires_grad = True)
        
        projector = partial(CombinedRDKronFiLM, seed=82,is_layered = True)
        self.P = projector(self.D, self.d, self.w, 'w_param')
    ...
    def forward(self, *args, **kwargs):
        ...
        flat_projected_params = (self.global_P @ weights) + self.P @self.w
...
def create_intrinsic_model(
    ...
    module = AIDModule if Test else IDModule

    if intrinsic_mode == "filmrdkron":
        class FiLMRDKronIDNet(module):
            def __init__(self, net, basis_num = basis_num, dimension=1000, seed=None,global_param = global_param, global_P = global_P, **_):
                super().__init__(
                    net, dimension=dimension , basis_num = basis_num,global_param = global_param, global_P = global_P
                )
        net = FiLMRDKronIDNet(base_net, basis_num = basis_num, dimension=intrinsic_dim, seed=seed,global_param = global_param, global_P = global_P
                             )
