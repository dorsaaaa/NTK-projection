...
def create_model(model_name=None, num_classes=None, base_width=None, in_chans=None,
  ...
  if log_dir is not None:
    ## Save initialization.
    #TODO maybe save meta model later
    #torch.save(base_net.state_dict(), Path(log_dir) / 'init_model.pt')
    #logging.info(f'Saved base model at "{Path(log_dir) / "init_model.pt"}".')
    pass

  ##create layer groups based on model structure:
  #groups = [4,4,4,4,4,4,4,4,4,2]
  #groups = [12,12,12,2]
  ## Create intrinsic dimensionality model.
  #commented this part t create same base net for meta learning
  '''
  final_net = base_net if intrinsic_cfg is None else \
              create_intrinsic_model(base_net, **intrinsic_cfg, ckpt_path=id_ckpt_path, device=device, is_said = is_said,is_layered = is_layered,groups = groups,global_param = global_param)
  ...
  if log_dir is not None:
    ...
    wandb.save('*.yml')
    #print("final net", final_net.subspace_params)
    '''
  '''
  return base_net
