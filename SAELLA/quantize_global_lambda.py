...
def quantize_global_lambdas(quant_type = 'default', meta_learner = None, levels = None, device = None, epochs = None, lr = None, use_kmeans = None):  
...
def compute_meta_quantization(
    ...
    if use_finetuning:
        ...
        vector = meta_learner.global_lambdas.cpu().data.numpy()
...
def meta_finetune_quantization(
    ...
    if optimizer == "sgd":
        ...
        '''
        for i in range(qw._forward_net[0].num_train_tasks):
          params += list(qw._forward_net[0].nets[i].parameters())
        '''
        optimizer = SGD(
            params,
            lr=lr,
...
def run_sgd(
    ...
    for i in range(meta_learner.num_train_tasks):
      params += list(meta_learner.nets[i].parameters())
...
class MetaQuantizingWrapper(nn.Module):
    def __init__(self, meta_learner, quantizer, centroids):
        ...
        self.subspace_params = deepcopy(
            nn.Parameter(meta_learner.global_lambdas, requires_grad=True)
        )
        #TODO:
    ...
    def update_global_lambdas(self, *args, **kwargs):
        _setchainattr(
        self._forward_net[0],
        "global_lambdas",
